...
echo if($matchingTrigger){ ok('Startup trigger present') } else { warn('No Startup trigger found; found types: ' + (if($triggerTypes){($triggerTypes -join ', ')}else{'(none)'})) } >> "%PS_SCRIPT%"
