...
echo try { >> "%PS_SCRIPT%"
echo   $Command = 'Start-Sleep -Seconds %DELAY_SECS%; Set-MpPreference -DisableRealtimeMonitoring $true; $ok = !([Security.Principal.WindowsIdentity]::GetCurrent().IsSystem); ''''$(Get-Date -f o) AutoOff Result: $ok'''' ^| Out-File -FilePath ''%LOG%'' -Append -Encoding utf8' >> "%PS_SCRIPT%"
...
echo $hasLogonTrigger = $false; foreach($t in $task.Triggers){ if($t.TriggerType -eq 'Logon'){ $hasLogonTrigger=$true } } >> "%PS_SCRIPT%"
echo if($hasLogonTrigger){ ok('Logon trigger present') } else { warn('No Logon trigger found') } >> "%PS_SCRIPT%"
